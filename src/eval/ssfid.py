# from https://github.com/Sin3DM/Sin3DM/blob/main/evaluation/ssfid.py
import numpy as np
import torch
...
def calculate_activation_statistics(voxel, model, model_out_layer=2):
    ...
    act = act.permute(0, 2, 3, 4, 1).view(-1,
                                          act.shape[1]).detach().cpu().numpy()  # (D*H*W, C)
...
def eval_SSFID_given_paths(data_paths, ref_path, model_out_layer=2, device="cpu"):
    ...
    ref_data = load_sdfgrid2vox(
        ref_path, resolution=128, device=device).float()

    mu_r, sigma_r = calculate_activation_statistics(
        ref_data, model, model_out_layer)
    ...
    for path in tqdm(data_paths, desc="SSFID"):
        ...
        if gen_data.shape != ref_data.shape:
            raise RuntimeError(
                'Generated shape and reference shape shall have equal size.')

        mu_f, sigma_f = calculate_activation_statistics(
            gen_data, model, model_out_layer)
