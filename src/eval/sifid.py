...
def calculate_sifid_given_paths(paths1, paths2, batch_size, cuda, dims, suffix, model=None):
    ...
    for i in range(len(paths2)):
        m1, s1 = calculate_activation_statistics(
            [paths1[i]], model, batch_size, dims, cuda)
        m2, s2 = calculate_activation_statistics(
            [paths2[i]], model, batch_size, dims, cuda)
...
def calculate_multiview_sifid_given_paths(gen_render_dirs, ref_render_dir, batch_size=1, device="cuda:0", dims=64):
    ...
    for i in tqdm(range(n_views), desc='Calculating multi-view SIFID'):
        ...
        gen_render_paths = [os.path.join(
            gen_render_dir, f'{i:03d}.png') for gen_render_dir in gen_render_dirs]
        assert len(gen_render_paths) > 0

        mu_ref, sigma_ref = calculate_activation_statistics(
            [ref_render_path], model, batch_size, dims, device)

        sifid_view_list = []
        for gen_path in gen_render_paths:
            mu_gen, sigma_gen = calculate_activation_statistics(
                [gen_path], model, batch_size, dims, device)
            sifid = calculate_frechet_distance(
