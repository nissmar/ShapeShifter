# from https://github.com/Sin3DM/Sin3DM/blob/main/evaluation/patch_utils.py
...
def load_sdfgrid2vox(path, binarize=True, resolution=128, device="cpu"):
    ...
    if max(sdfgrid.shape) != resolution:
        new_shape = [int(x * resolution / max(sdfgrid.shape))
                     for x in sdfgrid.shape]
...
def load_voxgrid(path, resolution=128, device="cpu"):
    ...
    if max(voxgrid.shape) != resolution:
        new_shape = [int(x * resolution / max(voxgrid.shape))
                     for x in voxgrid.shape]
        voxgrid = F.adaptive_max_pool3d(
            voxgrid[None, None].float(), new_shape)[0, 0].bool()
    return voxgrid
...
def extract_valid_patches_unfold(voxels: torch.Tensor, patch_size: int, stride=None):
    ...
    patches = voxels.unfold(0, patch_size, overlap).unfold(
        1, patch_size, overlap).unfold(2, patch_size, overlap)
    patches = patches.contiguous().view(-1, patch_size, patch_size,
                                        patch_size)  # (k, ps, ps, ps)
...
def eval_LP_IoU(gen_patches: torch.Tensor, ref_patches: torch.Tensor, threshold=0.95):
    ...
    for i in range(gen_patches.shape[0]):
        intersect = torch.logical_and(
            ref_patches, gen_patches[i:i+1]).sum(dim=(1, 2, 3))
...
def eval_LP_given_paths(data_paths, ref_path, patch_size=11, stride=5, patch_num=1000, device="cpu"):
    ...
    # load reference
    gen_data_shape = load_voxgrid(
        data_paths[0], resolution=128, device=device).shape
    ...
    for path in tqdm(data_paths, desc="LP-IOU/F-score"):
        ...
        gen_patches = extract_valid_patches_unfold(
            gen_data, patch_size, stride)
        ...
        lp_fscore_avg, lp_fscore_percent = eval_LP_Fscore(
            gen_patches, ref_patches)
