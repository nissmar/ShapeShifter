...
# def load_and_sample_shape(model_name: str, sample_n=1e5, rescale_f=True, return_normals=False):
#     """load and sample shape with normalization options"""
#     v, f = igl.read_triangle_mesh(model_name)
#     if rescale_f:
#         v = 2 * NDCnormalize(v)
#     if sample_n == 0:
#         return v, f
#     ref_mesh = trimesh.Trimesh(v, f)
#     samples, face_index = trimesh.sample.sample_surface_even(
#         ref_mesh, int(sample_n))
#     samples = np.array(samples)
#     if return_normals:
#         samples_normals = np.array(ref_mesh.face_normals[face_index])
#         return v, f, samples, samples_normals
#     return v, f, samples
...
# def export_ply_point_normal(name, vertices, normals=None, colors=None):
#     fout = open(name, 'w')
#     fout.write("ply\n")
...
#         if colors is None:
#             colors = (normals+1)/2.
#         colors = np.round(255*colors).clip(0, 255).astype(int)
#         for ii in range(len(vertices)):
#             fout.write(str(vertices[ii, 0])+" "+str(vertices[ii, 1])+" "+str(vertices[ii, 2])+" "
#                        + str(normals[ii, 0])+" "+str(normals[ii, 1]
#                                                      )+" "+str(normals[ii, 2])+" "
